...
namespace Inferno::Outrage {
    ...
    static Color Vector3Color(Vector3 v) {
    ...
    GenericInfo ReadGenericPage(StreamReader& r) {
        ...
        if (r.ReadByte())
            gen.Description = ReadString(r); // might contain \n
        ...
        if (version < 20 &&
            (gen.Type == ObjectType::Robot || gen.Type == ObjectType(16)/*ObjectType::Building*/) &&
